...
3. [Глоссарий (термины простыми словами)](#глоссарий-термины-простыми-словами)
4. [Архитектура и потоки данных](#архитектура-и-потоки-данных)
5. [Локальный стенд (Docker Compose)](#локальный-стенд-docker-compose)
6. [Конфигурация сервиса (`config.yaml`)](#конфигурация-сервиса-configyaml)
7. [Данные и доменные модели](#данные-и-доменные-модели)
8. [Дедупликация (Dedup)](#дедупликация-dedup)
9. [Оконный движок: кольцевые буферы + rolling-суммы + grace](#оконный-движок-кольцевые-буферы--rolling-суммы--grace)
10. [Снапшоты (Snapshot/Restore)](#снапшоты-snapshotrestore)
11. [Ingest из Redpanda/Kafka](#ingest-из-redpandakafka)
12. [Долговременное хранение: ClickHouse + суточные MV](#долговременное-хранение-clickhouse--суточные-mv)
13. [HTTP API + JWT + Rate Limiting](#http-api--jwt--rate-limiting)
14. [WebSocket + коалесинг + кластерный фан-аут (NATS)](#websocket--коалесинг--кластерный-фан-аут-nats)
15. [Наблюдаемость: метрики и pprof](#наблюдаемость-метрики-и-pprof)
16. [Переключение Redpanda ⇄ Kafka](#переключение-redpanda--kafka)
17. [Тест-план (unit/integration/load)](#тест-план-unitintegrationload)
18. [Runbook (операционные правила)](#runbook-операционные-правила)
19. [Быстрый старт (команды)](#быстрый-старт-команды)
...
## Глоссарий (термины простыми словами)
...
(В проде можно поднять Apache Kafka; код не меняется.)
...
services:
  ...
  clickhouse:
    ...
    environment: [CLICKHOUSE_DB=swaps]
...
**Что это:** периодическая сериализация **горячего состояния** (кольца + rolling) + **offsets** стрима в Redis/файл.
...
TTL event_date + INTERVAL 90 DAY TO DISK 'cold', event_date + INTERVAL 365 DAY DELETE;
```
...
```bash
./build/kafka/create_or_update_topic.sh \
...
```bash
CONFIG=cmd/aggregator/config.example.yaml go run ./cmd/aggregator
```
...
* **Snapshot/Restore** = быстрый тёплый старт, минимальный RTO.
...
> Готов продолжить: выбрать первый модуль для реализации (например, **оконный движок с тестами**) и дать “скелет” кода, в который сразу начнёшь добавлять логику.
